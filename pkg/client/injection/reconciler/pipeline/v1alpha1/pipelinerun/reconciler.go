...
import (
	...
	pipelinev1alpha1 "github.com/chengjoey/pipelines/pkg/client/listers/pipeline/v1alpha1"
	zap "go.uber.org/zap"
...
// controller reconciling v1alpha1.PipelineRun.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.PipelineRun. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.PipelineRun) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.PipelineRun.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.PipelineRun. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.PipelineRun) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.PipelineRun if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.PipelineRun.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.PipelineRun) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.PipelineRun) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.PipelineRun resources.
type reconcilerImpl struct {
	...
	Lister pipelinev1alpha1.PipelineRunLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister pipelinev1alpha1.PipelineRunLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1alpha1.PipelineRun, desired *v1alpha1.PipelineRun) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.ZchengjoeyV1alpha1().PipelineRuns(desired.Namespace)
		...
		updater := r.Client.ZchengjoeyV1alpha1().PipelineRuns(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.PipelineRun) (*v1alpha1.PipelineRun, error) {
	...
	patcher := r.Client.ZchengjoeyV1alpha1().PipelineRuns(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.PipelineRun) (*v1alpha1.PipelineRun, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.PipelineRun, reconcileEvent reconciler.Event) (*v1alpha1.PipelineRun, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
