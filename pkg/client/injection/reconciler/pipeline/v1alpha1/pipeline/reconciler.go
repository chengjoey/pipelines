...
// controller reconciling v1alpha1.Pipeline.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.Pipeline. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.Pipeline) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.Pipeline.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.Pipeline. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.Pipeline) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.Pipeline if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.Pipeline.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.Pipeline) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.Pipeline) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.Pipeline resources.
type reconcilerImpl struct {
	...
	Lister pipelinev1alpha1.PipelineLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister pipelinev1alpha1.PipelineLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.Pipeline) (*v1alpha1.Pipeline, error) {
	...
	patcher := r.Client.ZchengjoeyV1alpha1().Pipelines(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.Pipeline) (*v1alpha1.Pipeline, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.Pipeline, reconcileEvent reconciler.Event) (*v1alpha1.Pipeline, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
